...
| 09.EL.14 | `scripts/09-gh-pages-publish.sh`                             | authenticated gh-pages deploy [04] |
| 09.EL.15 | `.env`, `.env.sample`                                        | `GITHUB_PAT_FILE` param [04]   |
| 09.EL.16 | `scripts/07-deliverables-scaffold.sh`                        | status convention amendment [05] |
| 09.EL.17 | `docs/features/08-assessment-deliverables-conventions.md`    | status convention amendment [05] |
...
04. **09.EL.14/15** added for [09.IS.01](#validate) - the `gh-pages` push had no credentials available in this environment.
05. **09.EL.16/17** added for [09.IS.02](#validate) - moves deliverable `status` off the page and into the manifest, adding `open`.
