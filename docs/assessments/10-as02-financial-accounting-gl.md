...
**top-down/bottom-up reconciliation added per user direction** - three rows appended to the notebook's decomposition cell and the root-cause deliverable's findings table: the independent Ledger total variance (`gl_amount - txn_amount` from 10.04's write-back, reused rather than recomputed - 4,002,303.12), the bottom-up sum of the seven category contributions above (4,793,863.40), and the residual between them (-791,560.28). The residual does not close to zero and is reported as found: bottom-up exceeds top-down because **10.CK.21**/**10.CK.22** (legal-entity/cost-center misclassification) redistribute a transaction's value between sub-totals without changing the Ledger's grand total, so summing them alongside categories that do move the total (duplicates, unmapped transactions, FX errors) overstates what the bottom-up sum should be compared against - stated plainly in the deliverable rather than adjusted to close.
