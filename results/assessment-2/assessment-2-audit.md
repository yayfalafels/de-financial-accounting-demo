...
| 01.02-08 | recomputation/dim recon  | n/a [01]                           | 34 keys, 305,281.76 | n/a |

01. **01.02-08** has no single injected-issue tag of its own; two implementation issues were found and fixed while developing this check (`docs/assessments/10-as02-financial-accounting-gl.md`, issues `10.IS.02` and `10.IS.03`, have the full diagnostic trail for both). First, the recomputation summed `local_amount` where `finance.gl_balance`'s own movement figures are aggregated from `transaction_amount` (confirmed against the seed generator's source) - fixing the column alone produced a 0.00 result. Second, that 0.00 was itself still tautological: the recomputation grouped by each transaction's *actual* `gl_account`/`cost_center`/`legal_entity`, the same values the Ledger was built from, so a genuine misclassification could never surface. Grouping instead by the *expected* classification (`ref.accounting_mapping` where unambiguous, majority-vote for legal entity) surfaces the real variance: 34 keys, 305,281.76 - see the reconciliation-results and root-cause-analysis deliverables for the finding this produces and how much of it Task 3's own categories explain.
...
**bridge check** - Task 1's corrected recomputation (34 keys, 305,281.76) is checked directly against Task 3's own categories rather than left as a standalone figure: `03.07` + `03.08` at twice face value (60,697.50 + 85,485.13, x2 - a misposted transaction's value is missing from its correct bucket and present in its wrong one) = 292,365.26, 96% of 305,281.76. Confirmed the two categories' transactions don't overlap (a transaction wrong on both dimensions would break the simple x2 arithmetic) - zero overlap found. The remaining 12,916.50 traces to one specific transaction (`FTX-0000660`) that is wrong on cost center *and* is one of `02.01`'s two GL-mismatch rows unexplained by a mapping conflict - a multi-dimension interaction this investigation did not decompose further.
