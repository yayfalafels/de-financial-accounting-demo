...
Overall, the platform's total balance agrees with the General Ledger to the cent - no money is missing. The disagreement Finance is seeing is transactions landing in the wrong bucket, not money disappearing. About SGD 297,137 - roughly 30 of the 589 lines on the Ledger - was posted against the wrong legal entity, the wrong GL account, or the wrong cost center. Tracing 16 transactions back to where they should have posted closes that gap exactly to zero.
...
- **No accounting rule on file** - 385 transactions (SGD 4.39 million combined) have no rule to check them against at all. This is the largest figure by value, but it is a coverage gap, not a confirmed error - today, nobody can say whether these are posted correctly.
...
- put standing controls in place, not just a one-time fix:
    ...
    - check the exchange rate in force at the point of posting, not after
- move from a one-time check to a daily one, with clear pass/warning/fail thresholds, so this kind of drift is caught within a day instead of surfacing after it has built up across a reporting period
