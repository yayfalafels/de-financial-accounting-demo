...
- cited by notebook section, not a `reconciliation.rc_batch_control.batch_id` - task 2 findings are not written to `reconciliation.rc_*` (see the tracker's workflow cycle note 01)
- ground-truth verification: [assessment-2-audit.md](assessment-2-audit.md)
- unioned into the minimum-columns exception dataset: [assessment-2-exception-dataset.md](assessment-2-exception-dataset.md)

## Checks

| id       | check                                      | result            |
| -------- | --------------------------------------------- | -------------------- |
| 10.CK.09 | posted to expected GL account                 | 403 rows / 319 txns - `GL_MISMATCH` [01] |
...
| 10.CK.12 | overlapping effective-date mapping ranges     | 8 pairs, 6 (product, type) combos [02] |
| 10.CK.13 | expired mapping still referenced              | 0 - `EXPIRED_MAPPING`          |
| 10.CK.14 | product mapped to multiple GL accounts        | 4 combos [03]                  |
...
| product / type | rows | distinct transactions | explained by 10.CK.12/10.CK.14? |
| -------------- | ---- | ----------------------- | ------------------------------- |
...
All 385 rows fall on five `(product_code, debit_credit_indicator)` pairs that have **no row at all** in `ref.accounting_mapping` (`P7/CREDIT`=91, `P4/CREDIT`=85, `P2/DEBIT`=77, `P10/CREDIT`=74, `P6/CREDIT`=58) - `ref.accounting_mapping`'s 22 rows cover 15 of the 20 `(product, type)` combinations actually posted in `bronze.finance_transactions`; these five are structurally absent from the reference table rather than individually mis-tagged. No `issue-log.csv` tag names this directly (see [assessment-2-audit.md](assessment-2-audit.md)) - it is a mapping-table coverage characteristic of the seed data, not a per-row injected defect.
