...
| less: dimensional-only categories [05]        | -          | -146,182.63            |
| less: value double-counted across categories [06] | -      | -125,566.69            |
| **adjusted bottom-up sum**                    | -          | **4,522,114.08**       |
| **adjusted residual** [07]                    | -          | **-519,810.96**        |
...
04. **unexplained residual** = independent Ledger total variance - bottom-up sum. Negative here, meaning the categories together attribute *more* value than the single top-line variance.
05. **dimensional-only categories** - incorrect legal-entity allocation and incorrect cost-center assignment reassign a transaction between sub-totals without changing the Ledger's grand total, so their value should not have been counted toward the bottom-up sum in the first place; removing it is a correction, not an assumption.
06. **double-counted value** - measured directly, not assumed: the transactions flagged by duplicate/re-posted entry, incorrect FX conversion, and missing accounting mapping are pooled, and the value counted more than once (a transaction appearing in more than one of the three) is the gap between the sum with repeats and the sum over distinct transaction ids. All of it traces to the overlap between duplicate/re-posted entries and missing accounting mapping; no overlap was found between FX conversion and either of the other two.
07. **adjusted residual** = independent Ledger total variance - adjusted bottom-up sum. These two corrections account for 271,749.32 of the original 791,560.28 residual (34.3%) - the remaining 519,810.96 is a genuine unexplained residual, not closed by either predicted cause.
...
The bottom-up sum of category contributions (4,793,863.40) exceeds the independent top-line Ledger variance (4,002,303.12) by 791,560.28. Two specific, testable predictions were checked against that gap directly rather than assumed to explain it: removing the two dimensional-only categories (legal-entity and cost-center misclassification, which redistribute value between sub-totals without changing the Ledger's grand total) and removing the value double-counted where a transaction is flagged by more than one of the remaining checks (measured directly - all of it traces to overlap between duplicate/re-posted entries and missing accounting mapping) together account for 271,749.32 of the gap, 34.3%. The remaining 519,810.96 stays a genuine unexplained residual - not closed by either predicted cause, and not forced to zero. Separately, two of the eight named categories (incorrect debit/credit indicator, posted one accounting day late) returned no confirmed population from the methods used - this investigation cannot rule out that either is present but too small, or too entangled with a larger co-occurring cause at the same Ledger key, to isolate with the checks used here.
