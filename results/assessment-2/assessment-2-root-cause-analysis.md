...
| **total GL variance** (Task 1 recomputation) | 589 keys | **297,137.40**       |
| less: sum of explained factors, verified [06] | 16     | 297,137.40                |
| = **residual**                         | -           | **0.00**                  |
...
06. **sum of explained factors, verified** - legal-entity, GL-account, cost-center, and the one combined transaction (16 transactions total) are the only rows above Task 1's recomputation substitutes an expected value for, so they are the only candidates that can explain the total GL variance. Their combined contribution is measured directly - reverting exactly these 16 transactions to their actual, as-posted classification and re-running the recomputation - not by adding up their published face values (60,697.50 + 53,687.44 + 40,036.00 + 9,706.73 = 164,127.67) and doubling the total, the way the "missing from its correct bucket, present in its wrong one" rule of thumb would suggest (328,255.34). That face-value-doubled estimate overshoots the true figure by 31,117.94, because two of the four categories' transactions can share every other dimension of the same five-key posting bucket and net against each other there instead of adding cleanly - it is not a usable arithmetic check on its own. The direct, measured contribution of the 16 transactions is exactly 297,137.40 - the full total - confirmed by [Bridging the variance](#bridging-the-variance) below, where the same 16 transactions are reverted and the recomputation re-run to 0 keys / 0.00 variance.

## Bridging the variance

Task 1's expected-classification recomputation substitutes a different value for exactly 16 transactions across the three classification dimensions above (6 legal-entity, 6 GL-account, 3 cost-center, 1 both) - every other transaction's actual and expected classification already agree, contributing identically to both the Ledger and the recomputation. Re-running the recomputation with exactly these 16 transactions reverted to their actual, as-posted classification and every other transaction unchanged closes the bridge directly rather than by estimate:

| step                                                                 | amount     |
| ----------------------------------------------------------------------- | ------------ |
| Task 1 movement variance (expected-classification basis)                  | 297,137.40     |
| variance with the 16 flagged transactions reverted to actual classification | 0.00           |
| = residual                                                             | 0.00           |

0 of 589 keys exceed tolerance once those 16 transactions' classification is reverted - the full SGD 297,137.40 is fully and exactly explained by them, and by no other transaction or category. (A simpler twice-face-value estimate - summing each category's total and doubling it - overshoots the true figure by 31,117.94: two categories can coincidentally share every other dimension of the same five-key posting bucket, so their effects partly net out there rather than adding; the direct re-run above is the reconciliation actually used, not the estimate.) Two further transactions are confirmed, real misclassifications - one flagged by the indicator check, one by the cost-center check - that this reconciliation cannot bridge to at all, because both post under a product/transaction-type combination with more than one currently-active, conflicting mapping row, leaving no single unambiguous expected value to substitute either way (see finding 05 above and the mapping validation's overlapping-mapping finding).
